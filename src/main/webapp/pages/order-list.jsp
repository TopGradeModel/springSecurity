...
<body class="hold-transition skin-blue sidebar-mini">
							...
							<div class="form-group form-inline">
								<div class="btn-group">
									<button type="button" class="btn btn-default" title="新建"
										onclick='location.href="${pageContext.request.contextPath}/order/addToView"'>
						...
						<table id="dataList"
							...
							<c:forEach items="${list }" var="order">
								<tr>
									...
									<td>${order.orderStatus==1?"未支付":"已支付" }</td>
									<td>${order.product.productName}</td>
									
									<td class="text-center">
										<button type="button" class="btn bg-olive btn-xs"
											onclick='delOne(${order.id})'>删除</button>
										<button type="button" class="btn bg-olive btn-xs"
											onclick='location.href="${pageContext.request.contextPath}/order/findById?id=${order.id}"'>修改</button>
									</td>
								</tr>
	...
	function delOne(id){
			if(confirm("您确定要删除吗？")){
				//执行删除操作
				location.href="${pageContext.request.contextPath}/order/delOne?id="+id
			}
		}
	</script>
	<script
